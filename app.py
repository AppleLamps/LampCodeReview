...
def process\_uploaded\_files(
uploaded\_files: List[st.runtime.uploaded\_file\_manager.UploadedFile]
...
def construct\_user\_prompt(code\_contents: List[Dict[str, str]]) -\> str:
"""Formats the code content into a single string for the user prompt."""
prompt\_parts = ["Please review the following code files:\\n\\n"]
for item in code\_contents:
prompt\_parts.append(f"--- File: {item['filename']} ---\\n\\n`\n{item['content']}\n`\\n\\n")
return "".join(prompt\_parts)

def stream\_grok\_review(api\_key: str, user\_prompt: str) -\> Generator[str, None, None]:
"""
...
payload = {
    # FIX: Changed model name to a valid OpenRouter ID. "grok-4" is not a public model ID.
    # Using grok-1 as a powerful, available alternative.
    "model": "xai/grok-1",
    "messages": [
...
st.set\_page\_config(layout="wide", page\_title="CodeGuardian AI Review")
...
env\_api\_key = os.getenv("OPENROUTER\_API\_KEY")
if env\_api\_key:
api\_key = env\_api\_key
\# Updated message to be more generic
st.info("API key found in an environment variable.", icon="🔑")
else:
api\_key = st.text\_input("Enter your OpenRouter API Key", type="password")
...
\- Max total size: {MAX\_TOTAL\_SIZE // 1024\*\*2}MB. Larger files will be truncated.
""")

uploaded\_files = st.file\_uploader(
"Select code files or a .zip archive",
accept\_multiple\_files=True,
type=[ext.lstrip('.') for ext in SUPPORTED\_EXTS] + ['zip']
)

# \--- Analysis Execution ---

if st.button("🔍 Analyze Code", type="primary", use\_container\_width=True):
if not uploaded\_files:
st.warning("⚠️ Please upload code files to analyze.")
else:
with st.spinner("🔄 Preparing and processing files..."):
code\_contents, warnings = process\_uploaded\_files(uploaded\_files)
